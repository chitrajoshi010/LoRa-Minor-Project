...
- **Acoustic classifier** (I2S INMP441 -> mel-spectrogram -> TFLite Micro, 5
   ...
   task on the node (`main/acoustic/`).
...
└── main/
    ...
    ├── idf_component.yml         # radiolib, esp-tflite-micro, esp-dsp, esp-dht
    ...
    └── acoustic/                 # audio_capture, spectrogram, classifier (role 2)
```
...
```sh
# Load ESP-IDF tools in each new terminal session (fixes: idf.py: command not found)
. "$HOME/esp/esp-idf/export.sh"

# If the file above does not exist, install ESP-IDF first (one-time):
# mkdir -p "$HOME/esp"
# git clone -b v5.2.2 --recursive https://github.com/espressif/esp-idf.git "$HOME/esp/esp-idf"
# "$HOME/esp/esp-idf/install.sh"
# . "$HOME/esp/esp-idf/export.sh"

PROJECT_DIR="/home/chitra/Minor_Project/forest_monitor"

# Gateway (ESP32-WROOM-32)
cd "$PROJECT_DIR" && idf.py set-target esp32
...
# (or: cd "$PROJECT_DIR" && idf.py menuconfig -> "LDSE forest-monitor configuration" -> role = 1)
```
...
- **DHT component API:** `sensors/dht22.cpp` wraps the `chmorgan/esp-dht`
   component (setDHTgpio / readDHT / getTemperature / getHumidity). If the
   installed component exposes a different API, adjust that one file.
